package com.lpu.payments.controller;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.lpu.payments.entity.Payment;
import com.lpu.payments.service.PaymentService;

@RequestMapping("/payment")
@RestController
public class PaymentController {

	@Autowired
	private PaymentService service;
	
	@PostMapping("/save")
	public Payment savePayment(@RequestBody Payment payment) {
		return service.savePayment(payment);
	}
	
	@GetMapping("/find/{paymentId}")
	public Payment findPayment(@PathVariable int paymentId) {
		return service.findPaymentById(paymentId);
	}
	
	@GetMapping("/findAll")
	public List<Payment> findAll() {
		return service.findAllPayments();
	}
	
	@PutMapping("/update/{paymentId}")
	public Payment updatePayment(@PathVariable int paymentId, @RequestBody Payment payment) {
		return service.updatePaymentById(paymentId, payment);
	}
	
	@DeleteMapping("/delete/{paymentId}")
	public void deletePayment(@PathVariable int paymentId) {
		service.deletePaymentById(paymentId);
	}
	
}
